    ...
    return (
        <Link to={`/book/${bookId}`} className="border border-gray-200 p-5 rounded-xl">
            ...
            <ul className="flex justify-start mb-3">
                {tags.map(li => <li key={li} className="rounded-full bg-gray-100 text-sm py-1 px-2 me-1 ">{li}</li>)}
            </ul>
            ...
            <div className="flex justify-between">
                <p>{category}</p>
                <p className="flex justify-end items-center gap-1"><span>{rating}</span> <FaRegStar /></p>
            </div>
        </Link>
    )
}
